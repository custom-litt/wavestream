...
/** buffer where chunks of sound file will be converted to floats to be sent
...
int main(int argc, char** argv) {
  ...
  // by default this will try backends in order and select the first that is
  // successful
  enum SoundIoBackend backend = SoundIoBackendNone;
  ret = (backend == SoundIoBackendNone)
            ? soundio_connect(soundio)
            : soundio_connect_backend(soundio, backend);
  if (0 != ret) {
    fprintf(
        stderr, "Unable to connect to backend: %s\n", soundio_strerror(ret));
    return 1;
  }
  printf("Backend: %s\n", soundio_backend_name(soundio->current_backend));

  // with an up-to-date list of output devices select the first that works with
  // the backend
  soundio_flush_events(soundio);  // not sure why this is necessary - I think it
                                  // is meant to drive actions such as updating
                                  // lists of connected devices etc...
  ...
  if (outstream->layout_error) {
    fprintf(
        stderr, "unable to set channel layout: %s\n",
...
static void write_callback(
  ...
  for (;;) {
    ...
    for (int frame = 0; frame < frame_count; frame += 1) {
      double sample =
          snd_buf[frame * sfinfo.channels + 0];  // get the sample from just one
                                                 // channel of the input
                                                 // audio... this could later be
                                                 // improved
